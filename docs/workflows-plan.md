...
4. **Real infrastructure gate (passed 2026-09-23):** `cloud/runner/tests/vm-integration.sh` passed 79/79 checks on the dedicated disposable VM `ph-int-test` (created for the run, destroyed afterwards), including the script-job sections: unprivileged execution, durable bounded output, idempotent duplicate submission, nonzero-exit failure and detached-child cancellation. A clean runner snapshot was published from a fresh swept build VM as `powerhouse-base` **v2** (9.8 GB) with `probe.script_protocol_version == 3`. The coordinator's end-to-end acceptance through `BoxdExecutionAdapter` (`server/tests/e2e-boxd.test.ts`, opt-in `RUN_BOXD_E2E=1` + `BOXD_API_KEY`) passed all three scenarios against real boxd microVMs and a public repository: two scripts across two VMs with a coordinator restart between the nodes recovering without duplicates, a failing script blocking the second and still cleaning up, and cancellation confirming process termination before VM release — with zero `ph-wf-*` machines left afterwards. The gate surfaced and fixed a real recovery defect: an instance orphaned by a coordinator restart must never checkpoint error-path steps (that corrupts DBOS's deterministic step sequence) nor write failure projections (only the live coordinator's instance may), see `server/src/workflow.ts`.
5. **Implemented (increment 1, manual sequences):** published workflows with append-only immutable versions (`workflows`, `workflow_versions`), `POST/GET /v1/workflows`, `POST /v1/workflows/:id/versions`, and `POST /v1/workflows/:id/runs`, which pins the version's node list into the run at admission — publishing later versions never alters an active or past run. The DBOS sequence interpreter executes 1–10 named script nodes in order with the same durable identities, fail-fast, polling and cleanup semantics proven in 0b; ad-hoc `POST /v1/runs` still accepts exactly two scripts and now stores them as a node list.
6. **Still open:** where the coordinator's Postgres lives in deployment (shared backend VM with external backups versus managed hosting) and the artifact-storage provider. **Next implementation increment:** the live run UI transport (increment 2) — snapshot + SSE, run history, cancel from the desktop.
...
| 0b. Remote execution proof (done, including real-VM acceptance) | Pin DBOS and boxd SDKs; minimal authenticated backend/Postgres; reuse the runner; own VM lifecycle and submit/status/cancel from the server. | Restart the orchestrator after machine creation and after job acceptance but before recording each response. Recovery finds the original resources; cancellation confirms process termination. Proven with `FakeExecutionAdapter` and repeated on real boxd infrastructure (runner snapshot v2). |
| 1. Manual sequences (implemented server-side) | Shared schemas, migrations, immutable versions, run admission/reconciliation, script nodes and owner authentication (artifact references remain later scope). | Duplicate start requests create one run; a sequence keeps completed results after restart; a failed script blocks the next script. All covered by `server/tests/sequences.test.ts`; version immutability (a pinned run survives publishing v2 unchanged) is tested explicitly. |
