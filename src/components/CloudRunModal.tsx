...
import { cloudInspectSource, cloudProbeBase, cloudSubmit, type ProbeInfo, type SourceInfo } from "../lib/cloud";
...
export function CloudRunModal() {
  ...
  const canSubmit = !!task.trim() && !!source && problems.length === 0 && !busy && !!cloud.baseVm.trim();
  ...
  return (
    ...
    >
      ...
      >
        ...
        {probe && (
          <p className="text-xs text-muted-foreground">
            Runner {probe.runner_version} (protocol {probe.protocol_version}) · Claude{" "}
            {probe.claude_version ?? "not installed"} · credentials: Claude {probe.credentials.claude ? "✓" : "missing"}, Git publish{" "}
            {probe.credentials.git_publish ? "✓" : "missing"}
          </p>
        )}
